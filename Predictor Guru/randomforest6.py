...
for name, model in models:
    ...
    print(f"{name} CV MAE: {mean_cv_score:.2f} (+/- {cv_scores.std() * 2:.2f})")
...
print(f"Best model: {type(best_model.named_steps['regressor']).__name__}")

# Train the best model on the full dataset
best_model.fit(X, y)

# Save the best model
with open('best_rank_predictor_model.pkl', 'wb') as file:
...
with open('best_rank_predictor_model.pkl', 'rb') as file:
